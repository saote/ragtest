...
class LocalSearchMixedContext(LocalContextBuilder):
    ...
    def build_context_with_entities(
        self,
        ...
        **kwargs: dict[str, Any],
        ...
        """
        !!! Added function!!!
        Build data context for a list of selected entities.
    ...
    def _build_text_unit_context(
        ...
        ### modified
        ### sort selected text units by l2 similarity with query embedding
        if query:
            selected_text_units.sort(
                key=lambda x: self.l2_distance_of_texts(x.text, query)
            )
    ...
    ### modified
    def l2_distance_of_texts(self, p: str, q: str) -> float:
        p_embed = self.text_embedder.embed(p)
        q_embed = self.text_embedder.embed(q)
        return sum((px - qx) ** 2 for px, qx in zip(p_embed, q_embed))